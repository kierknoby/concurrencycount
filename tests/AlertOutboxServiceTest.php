...
$outbox = $service->lease($outbox, $eventId, 100);
outbox_assert_same('delivering', $outbox[$eventId]['delivery_status'], 'Lease marks event delivering');
outbox_assert_same(null, $service->nextReady($outbox, 159), 'Active lease is not selected');
outbox_assert_same($eventId, $service->nextReady($outbox, 160)['event_id'], 'Stale delivering lease becomes retryable');
...
outbox_assert_same('retry', $outbox[$eventId]['delivery_status'], 'Failed delivery leaves deterministic retry state');
outbox_assert_same('mailer unavailable', $outbox[$eventId]['last_error'], 'Failed delivery retains useful error');
