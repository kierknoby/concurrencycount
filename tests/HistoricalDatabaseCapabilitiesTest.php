...
capabilities_assert(!$legacy['cleanup_statement_timeout_supported'] && $legacy['adaptive_legacy_acquisition'] && $legacy['acquisition_window_seconds'] === 900, 'MariaDB 5.5 must use adaptive acquisition and reject Demo cleanup');

$modernMaria = HistoricalDatabaseCapabilities::fromServerVersion('10.6.18-MariaDB-0ubuntu0.22.04.1');
capabilities_assert($modernMaria['numeric_version'] === '10.6.18' && $modernMaria['select_statement_timeout_type'] === 'max_statement_time' && $modernMaria['cleanup_statement_timeout_supported'], 'Modern MariaDB must protect Historical SELECT and Demo cleanup with max_statement_time');
...
capabilities_assert($mysql57['vendor'] === 'mysql' && $mysql57['historical_supported'] && $mysql57['select_statement_timeout_type'] === 'max_execution_time' && !$mysql57['cleanup_statement_timeout_supported'], 'MySQL 5.7.8 must protect Historical SELECT but not claim cleanup DELETE protection');
