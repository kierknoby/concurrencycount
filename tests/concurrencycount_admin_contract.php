...
admin_contract_assert(strpos($chartJavascript, "value === null") !== false && strpos($chartJavascript, 'previousVisible = null') !== false, 'Historical chart must render below-floor points as gaps rather than bridging hidden values');
admin_contract_assert(strpos($css, '.cc-historical-graph canvas') !== false && strpos($css, 'height: 220px') !== false, 'Historical chart must have an explicit responsive CSS sizing contract');
admin_contract_assert(strpos($chartJavascript, 'if (!width || !height) return false') !== false && strpos($chartJavascript, '__ccConcurrencyChart') !== false, 'Chart lifecycle must defer hidden sizing and enforce one owner per canvas');
admin_contract_assert(strpos($liveJavascript, 'CCChartRenderScheduler') !== false && strpos($liveJavascript, "typeof root.requestAnimationFrame === 'function'") !== false && strpos($liveJavascript, 'historicalChartRenderScheduler.schedule') !== false, 'Historical chart rendering must use its explicit animation-frame scheduler with fallback');
admin_contract_assert(strpos($view, 'id="cc-historical-graph-loading"') !== false && strpos($view, 'Loading graph...') !== false && strpos($liveJavascript, "addClass('is-loading')") !== false && strpos($liveJavascript, "removeClass('is-loading')") !== false, 'Historical graph must remain in an explicit loading state until stable rendering completes');
admin_contract_assert(strpos($liveJavascript, 'requestedGraph.start_ts') !== false && strpos($liveJavascript, 'requestedGraph.end_ts') !== false && strpos($liveJavascript, 'stableDimensions') !== false && strpos($liveJavascript, 'canvas.__ccConcurrencyChart === charts.historical') !== false, 'Historical rendering must use the selected report domain and verify stable dimensions plus active canvas ownership');
admin_contract_assert(strpos($chartJavascript, 'formatAxisTimestamp') !== false && strpos($chartJavascript, 'this.domain ? this.domain.minTs') !== false, 'Historical chart axes must adapt labels while retaining the explicit selected-window domain');
