...
foreach (['wizardstep', 'run', 'demopreflight', 'getdemoscenario', 'savedemoscenario', 'cancelcalculation', 'calculationheartbeat', 'calculationtelemetry', 'peakdetails', 'livestatus', 'getsettings', 'savesettings', 'monitorstatus', 'restartmonitor', 'historicalgraph', 'download', 'previewfixture', 'email', 'gettrunks', 'listhistoricalreports', 'createhistoricalreport', 'updatehistoricalreport', 'closehistoricalreport', 'activatehistoricalreport', 'getidentityclassifications', 'saveidentityclassification', 'resetidentityclassification', 'resetallidentityclassifications', 'listexcludedcalls', 'excludecall', 'restoreexcludedcall', 'restoreallexcludedcalls'] as $command) {
...
admin_contract_assert(strpos($view, 'name="cc-demo-load"') !== false && strpos($view, "'medium' ? ' active'") !== false && strpos($view, 'data-toggle="buttons"') === false && strpos($view, 'aria-pressed="<?php echo') !== false && strpos($view, 'id="cc-demo-randomise"') !== false && strpos($view, 'id="cc-demo-save"') !== false, 'Demo must expose a JS-controlled load selector with deterministic visual and accessible Medium default');
admin_contract_assert(strpos($javascript, 'function renderDemoLoadSelection(load)') !== false && strpos($javascript, 'CCDemoScenario.loadState(load)') !== false && strpos($javascript, 'applyDemoPlan(window.CCDemoScenario.build(demoPlan.seed, load), false)') !== false, 'Demo load clicks and restored plans must share one explicit visual-state renderer while retaining the current seed');
...
admin_contract_assert(strpos($javascript, "command: 'getdemoscenario'") !== false && strpos($javascript, "command: 'savedemoscenario'") !== false && strpos($class, 'DEMO_SCENARIO_KEY') !== false, 'Demo scenario definitions must save and restore through server-side module persistence');
admin_contract_assert(strpos($javascript, ".fail(function () { showDemoError('Demo preflight request failed.") !== false && strpos($view, 'id="cc-demo-error"') !== false, 'Demo preflight transport failures must be visible inside the open modal');
...
admin_contract_assert(strpos($runDemoBody, 'CCDemoScenario.runParameters(demoPlan, report, engines, minimumConcurrency)') !== false, 'Each Demo mode must send its displayed scenario, selected engines, load and seed through the tested request seam');
admin_contract_assert(strpos($runDemoBody, ".always(function () { $('.cc-demo-run-mode').prop('disabled', false); updateDemoSelectionStatus();") !== false, 'Every Demo preflight completion path must restore the Run controls and Saved/Unsaved scenario status');
