...
admin_contract_assert(strpos($view, '<svg id="cc-historical-chart"') !== false && strpos($view, 'historical-svg-chart.js') !== false && strpos($css, '.concurrencycount #cc-historical-chart') !== false, 'Historical must size only its root responsive SVG to the explicit 220px visual height');
admin_contract_assert(strpos($css, '.cc-historical-graph svg') === false && strpos($css, '.cc-historical-svg-plot') === false, 'Root SVG sizing CSS must not override the nested Historical plot dimensions');
admin_contract_assert(strpos($view, '<svg id="cc-historical-chart" width="100%" height="220"') !== false && strpos($historicalSvgJavascript, 'class="cc-historical-svg-plot"') !== false && strpos($historicalSvgJavascript, 'width="93.5%"') !== false && strpos($historicalExportJavascript, 'preserveAspectRatio="xMidYMid meet"') !== false, 'On-screen plot geometry must fill responsive width without stretching text, while exported SVG uses uniform scaling');
admin_contract_assert(strpos($historicalSvgJavascript, 'var plotLeft = rect.left + (rect.width * 0.05), plotWidth = rect.width * 0.935') !== false && strpos($historicalSvgJavascript, '(event.clientX - plotLeft) / plotWidth') !== false, 'Historical pointer mapping must use the actual responsive plot rectangle and margins');
admin_contract_assert(strpos($historicalSvgJavascript, "point.value === null") !== false && strpos($historicalSvgJavascript, "commands.push('H ' + x(point.ts))") !== false, 'Historical SVG must end visible step runs at real null-boundary timestamps');
admin_contract_assert(strpos($historicalSvgJavascript, 'function markersForWidth') !== false && strpos($historicalSvgJavascript, 'cc-historical-svg-short-run') !== false && strpos($historicalSvgJavascript, 'data-start-ts=') !== false && strpos($historicalSvgJavascript, 'data-end-ts=') !== false, 'Sub-pixel Historical runs must use a separate display-marker layer carrying exact real boundaries');
admin_contract_assert(strpos($historicalSvgJavascript, 'currentRun.startTs + ((endTs - currentRun.startTs) / 2)') !== false && strpos($historicalSvgJavascript, 'minimumPixels / plotPixels') !== false, 'Short-run markers must anchor on real run time and derive display width from the current responsive plot width');
...
admin_contract_assert(strpos($liveJavascript, 'historicalSelectedSeries') !== false && strpos($liveJavascript, 'HistoricalGraphExport.download') !== false && strpos($liveJavascript, 'historicalChart.chart') !== false, 'Graph export must use the currently selected already-rendered Historical series');
admin_contract_assert(strpos($historicalExportJavascript, 'image/svg+xml') !== false && strpos($historicalExportJavascript, '<style>') !== false && strpos($historicalExportJavascript, 'cc-historical-svg-series') !== false && strpos($historicalExportJavascript, 'cc-historical-svg-short-run') !== false, 'SVG export must remain self-contained vector markup including short-run markers');
admin_contract_assert(strpos($historicalExportJavascript, "document.createElement('canvas')") !== false && strpos($historicalExportJavascript, 'canvas.width = EXPORT_WIDTH * 2') !== false && strpos($historicalExportJavascript, "format === 'jpeg' ? 'image/jpeg' : 'image/png'") !== false, 'PNG and JPEG must rasterise the SVG only inside the export path at two-times resolution');
