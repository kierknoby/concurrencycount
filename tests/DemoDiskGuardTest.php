...
class DiskDb { public $host; public $stats; public $logBin; public $logPath; public $indexes; public $indexError; public function __construct($stats = null, $logBin = 0, $logPath = '', $indexes = null, $indexError = false) { $this->host = gethostname(); $this->stats = $stats; $this->logBin = $logBin; $this->logPath = $logPath; $this->indexes = $indexes === null ? [['INDEX_NAME' => 'accountcode', 'SEQ_IN_INDEX' => 1, 'COLUMN_NAME' => 'accountcode']] : $indexes; $this->indexError = $indexError; } public function query($sql) { if ($this->indexError && strpos($sql, 'information_schema.STATISTICS') !== false) throw new RuntimeException('metadata unavailable'); return new DiskStmt($this, $sql); } }
class DiskStmt { private $db; private $sql; public function __construct($db, $sql) { $this->db = $db; $this->sql = $sql; } public function fetch($mode = null) { if (strpos($this->sql, 'information_schema.TABLES') !== false) return $this->db->stats; return ['datadir' => '/db', 'hostname' => $this->db->host, 'log_bin' => $this->db->logBin, 'log_bin_basename' => $this->db->logPath]; } public function fetchAll($mode = null) { return $this->db->indexes; } }
...
$statsGuard = new DemoDiskGuard(new DiskDb(['DATA_LENGTH' => 1000000, 'INDEX_LENGTH' => 1000000, 'TABLE_ROWS' => 100], 1, '/db/mysql-bin'), $space);
