(function (root, factory) {
	...
	function fingerprint(plan) { return [plan.token, plan.generation, plan.size, plan.year, plan.rows, plan.start, plan.end].join('|'); }
	function isLeapYear(year) { return (year % 4 === 0 && year % 100 !== 0) || year % 400 === 0; }
	function normaliseYear(year) {
		year = Math.round(Number(year));
		if (!Number.isFinite(year)) year = YEAR_DEFAULT;
	...
	function restore(definition) {
		...
		var plan = build({token:definition.token,generation:definition.generation}, definition.size, definition.year);
	...
	function equal(left, right) { return !!left && !!right && ['token', 'generation', 'seed', 'size', 'year', 'rows', 'start', 'end'].every(function (key) { return String(left[key]) === String(right[key]); }); }
	...
	function randomiser(randomBytes) {
		...
		return {next: function (load, year) { for (;;) { generation++; var bytes = randomBytes(16), token = Array.prototype.map.call(bytes, function (byte) { return ('0' + Number(byte).toString(16)).slice(-2); }).join(''); var plan = build({token:token,generation:generation},load,year); if (!seen[plan.fingerprint]) { seen[plan.fingerprint]=true; return plan; } }} };
	}
	...
	return {build: build, restore: restore, equal: equal, fingerprint: fingerprint, page: page, preflightGuard: preflightGuard, randomiser: randomiser, runParameters: runParameters, loads: LOADS, YEAR_MIN: YEAR_MIN, YEAR_MAX: YEAR_MAX, YEAR_DEFAULT: YEAR_DEFAULT};
}));
